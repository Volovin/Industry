    ...
    public class WindowsController : Singleton<WindowsController>
    {
        ...
        public sealed class WindowsCollection
        {
            public WindowsCollection()
            {
                ...
                RouteManager = windows.GetComponentInChildren<RouteManagerWindow>(true);

                var bGroups = windows.GetComponentsInChildren<BuildingsGroupWindow>(true);

                RawResources = bGroups[0];
                ResourcesProcessing = bGroups[1];
                ProductFactories = bGroups[2];
                UnloadingPoints = bGroups[3];
                Depots = bGroups[4];
